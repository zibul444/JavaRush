...
public class Solution {
    ...
    private static Map<String, Integer> countWords(ArrayList<String> list) {
        ...
        for (int i = 0; i < list.size(); i++){//вычисляем сколько раз повторяется, и фиксируем в MAP
            int counter = 0, counterIgnore = 0;
            int[]neverReadLess = new int[list.size()-1];
            for (int y = 0; y < list.size(); y++) {
                if (list.get(i).equals(list.get(y))) {
                    counter++;
                    neverReadLess[counterIgnore] = y;
                    counterIgnore++;
                }
            }
            if(counter>0){
